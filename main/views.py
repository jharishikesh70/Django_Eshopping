...
def index(request):
    ...
    Feature = Product.objects.filter(is_featured=True).order_by('-product_id')

    Popular = Product.objects.all().order_by('-num_visits')[0:6]
    Recently_viewed = Product.objects.all().order_by('-last_visit')[0:6]
...
def productView(request, prod_id):
    ...
    prod = Product.objects.filter(product_id=prod_id).first()
    prod.num_visits = prod.num_visits + 1
    prod.last_visit = datetime.now()
    prod.save()

    product = Product.objects.get(product_id=prod_id)
    related_products = Product.objects.filter(category=product.category).exclude(product_id=prod_id)[:6]
    params = {
        'product': prod,
...
def MyOrders(request):
    if request.method == 'POST':
        ...
        o.save()
    params = {
        'orders': [i for i in Orders.objects.all() if
                   i.user == request.user and i.status != 'Delivered' and i.status != 'Cancel'],
...
def MenuFilter(request, querys):
    ...
    prod = []
    for p in [i for i in Product.objects.all() if
              str(i.category).lower() == querys.split(',')[0].lower() and str(i.subcategory).lower() ==
              querys.split(',')[1].lower()]:
        prod.append([p, [item for item in ProductSize.objects.filter(product=p)]])
    params = {
        'product': prod,
        'catg': querys,
...
def contact(request):
    ...
    return render(request, 'main/contact.html', {'category': category.objects.all(), 'cart_element_no': len(
        [p for p in Cart.objects.all() if p.user == request.user]), })


def disease(request):
    info2 = Disease.objects.all()
    bookdata = {"details": info2}
    return render(request, 'main/disease.html', bookdata)


def footer(request):
    return render(request, 'main/footer.html', params)
